...
// Step 2 - Inject the Event Explorer markup
...
eventPanel.innerHTML = `
  <section class="event-lab">
    ...
    <ul id="tag-list" class="tags">
      <li data-tag="js">JavaScript</li>
      <li data-tag="css">CSS</li>
      <li data-tag="html">HTML</li>
    </ul>
    <p id="tag-output" aria-live="polite"></p>

  </section>
  `

// Step 3 - Set up logging helper
...
// Step 4 - Implement direct listeners mode
...
// Step 5 - Implement delegated listener mode
...
const tagOutput = document.querySelector<HTMLElement>("#tag-output");

if (tagList && tagOutput) {
  tagList.addEventListener("click", (event) => {
    const target = event.target as HTMLElement | null;
    const li = target?.closest<HTMLLIElement>("li");
    if (!li) return;
    const tag = li.dataset.tag ?? li.textContent?.trim() ?? "";
    tagOutput.textContent = `You clicked tag: ${tag}`
  })
}
